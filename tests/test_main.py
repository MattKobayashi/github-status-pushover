...
def test_check_feed_new_entries(mock_feedparser):
    ...
    # Setup test data
    old_entry = mock.Mock(published_parsed=(2023, 1, 1, 10, 0, 0, 0, 0, 0),
                         title="Old", description="Old issue", link="old")
    new_entry = mock.Mock(published_parsed=(2023, 1, 1, 12, 0, 0, 0, 0, 0),
                         title="New", description="New issue", link="new")
    
    # Mock feedparser response
    mock_feedparser.parse.return_value.entries = [new_entry, old_entry]
    
    with mock.patch('main.get_last_checked_time', 
